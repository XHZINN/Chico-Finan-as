...
function LinhaItem({ item, stamps, deletavelOrigens, mesFechado }) {
  return (
...
function LinhaGrupo({ grupo, stamps, deletavelOrigens, mesFechado }) {
  if (grupo.itens.length === 1) {
    return <LinhaItem item={grupo.itens[0]} stamps={stamps} deletavelOrigens={deletavelOrigens} mesFechado={mesFechado} />;
  }
  return (
    <details>
      ...
      <div className="group-items">
        {grupo.itens.map(item => (
          <LinhaItem key={item.id_transacao} item={item} stamps={stamps} deletavelOrigens={deletavelOrigens} mesFechado={mesFechado} />
        ))}
      </div>
    </details>
  );
...
export default function ListaTransacoes({ items, stamps, deletavelOrigens, mesFechado, vazioTexto }) {
  if (items.length === 0) {
  ...
  return (
    <>
      {visiveis.map((grupo) => (
        <LinhaGrupo key={grupo.itens[0].id_transacao} grupo={grupo} stamps={stamps} deletavelOrigens={deletavelOrigens} mesFechado={mesFechado} />
      ))}
      {restantes.length > 0 && (
        <details>
